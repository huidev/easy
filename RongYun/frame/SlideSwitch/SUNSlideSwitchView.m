...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSetRGBStrokeColor(context, 134.0/255, 27/255.0, 25/255.0, 1);//线条颜色
    CGContextSetLineCap(context, kCGLineCapRound);
//    CGContextSetLineWidth(UIGraphicsGetCurrentContext(), 15.0);  //线宽
    CGContextMoveToPoint(context, 0, kHeightOfTopScrollView);
    CGContextAddLineToPoint(context, self.bounds.size.width,kHeightOfTopScrollView);
...
//        UIViewController *vc = _viewArray[i];
        ...
        NSString *titleStr = [self.slideSwitchViewDelegate slideSwitchView:self titleOfTab:i];
        CGSize textSize = [titleStr sizeWithFont:[UIFont systemFontOfSize:kFontSizeOfTabButton]
                               constrainedToSize:CGSizeMake(_topScrollView.bounds.size.width, kHeightOfTopScrollView)
                                   lineBreakMode:NSLineBreakByTruncatingTail];
        //累计每个tab文字的长度
        textSize.width = MAX((SCREENWIDTH-(_viewArray.count+1)*kWidthOfButtonMargin)/_viewArray.count, textSize.width);
...
//                                    textSize.width, kHeightOfTopScrollView)];
//        //累计每个tab文字的长度
//        topScrollViewContentWidth = (SCREENWIDTH/_viewArray.count>(kWidthOfButtonMargin+textSize.width)?(SCREENWIDTH/_viewArray.count):(kWidthOfButtonMargin+textSize.width));
//        //计算下一个tab的x偏移量
//        xOffset += topScrollViewContentWidth;
//        
//        [button setTag:i+100];
//        if (i == 0) {
//            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 0, textSize.width, _shadowImage.size.height);
