...
#import "SetBBSInfoVC.h"
#import "PhotoSingleton.h"
@implementation DoImagePickerController1
...
- (void)viewDidLoad
{
    ...
    [self initControls];
    if (_type == 1) {
        UIBarButtonItem *rightBtn = [[UIBarButtonItem alloc] initWithTitle:@"下一步" style:UIBarButtonItemStylePlain target:self action:@selector(ok)];
        self.navigationItem.rightBarButtonItem = rightBtn;

    }else{
        UIBarButtonItem *rightBtn = [[UIBarButtonItem alloc] initWithTitle:@"完成" style:UIBarButtonItemStylePlain target:self action:@selector(ok)];
        self.navigationItem.rightBarButtonItem = rightBtn;
    }
    
    if (_type == 1) {
        _typeLab.text = @"选择论坛头像";
    }else{
        self.title = @"新建相册";
...
- (void)ok{
    ...
    if (_type == 1) {
        SetBBSInfoVC *info = [[SetBBSInfoVC alloc] init];
        [self.navigationController pushViewController:info animated:YES];
    }else{
        [PhotoSingleton sharedManager].beginUpload = YES;
...
- (IBAction)onSelectPhoto:(id)sender
{
    ...
    [_delegate didSelectPhotosFromDoImagePickerController:self result:aResult andDic:nil];
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        DoPhotoCell *oldCell;
        if (oldIndex < 1000) {
            oldCell = (DoPhotoCell *)[collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:oldIndex inSection:0]];
        }
        
        
        if (_nResultType == DO_PICKER_RESULT_UIIMAGE){
            //[_delegate didSelectPhotosFromDoImagePickerController:self result:@[[ASSETHELPER getImageAtIndex:indexPath.row type:ASSET_PHOTO_SCREEN_SIZE]] andDic: _dSelected];
            
            if (oldIndex == 1000) {
                [cell setSelectMode:YES];
            }else{
                [oldCell setSelectMode:NO];
                [cell setSelectMode:YES];
            }
            oldIndex = indexPath.row;
            NSMutableArray *arr = [[NSMutableArray alloc] initWithArray:[PhotoSingleton sharedManager].photoArr];
            NSArray *arr2 = @[[ASSETHELPER getImageAtIndex:indexPath.row type:ASSET_PHOTO_SCREEN_SIZE]];
            [arr insertObject:arr2[0] atIndex:0];
            [PhotoSingleton sharedManager].photoArr = arr;
        }
        else{
            [_delegate didSelectPhotosFromDoImagePickerController:self result:@[[ASSETHELPER getAssetAtIndex:indexPath.row]] andDic:_dSelected];
        }
