...
- (void)viewDidLoad
{
    ...
    _coverImage = [[UIImage alloc] init];
...
//    bgView.frame = CGRectMake(0, 0, MY_WIDTH, 64);
//    bgView.backgroundColor = BGColor;
//    
//    UILabel* titleLabel = [[UILabel alloc] init];
//    titleLabel.frame = CGRectMake((MY_WIDTH-200)/2, 20+7, 200, 30);
...
//    [leftBtn setTitleColor:RGB(17, 105, 201) forState:UIControlStateHighlighted];
...
{
    [_dSelected removeAllObjects];
    int i=1;
    for (NSDictionary *item in _selectedKeys)
    {
        NSString *key = [item allKeys][0];
        if (key.length <3) {
            [_dSelected setObject:[NSString stringWithFormat:@"%d",i] forKey:key];
            i++;
        }
    }
...
//    _vBottomMenu.backgroundColor = DO_MENU_BACK_COLOR;
    _vBottomMenu.backgroundColor = [UIColor grayColor] ;
...
//    NSArray *aKeys = [_dSelected keysSortedByValueUsingSelector:@selector(compare:)];
    ...
    if (_nResultType == DO_PICKER_RESULT_UIIMAGE)
    {
        ...
        {
            UIImage *imageS = [ASSETHELPER getImageAtIndex:[aKeys[i] integerValue] type:ASSET_PHOTO_SCREEN_SIZE];
            
            
            if (imageS) {
                [aResult addObject:@{[NSString stringWithFormat:@"%@",aKeys[i]]:imageS}];
                [photoArr addObject:imageS];
            }
    ...
    //[_delegate didSelectPhotosFromDoImagePickerController:self result:aResult :nil];
    [PhotoSingleton  sharedManager].photoArr = photoArr;
    
    DoImagePickerController1 *photoPicker = [[DoImagePickerController1 alloc] initWithNibName:@"DoImagePickerController1" bundle:nil];
    photoPicker.delegate = self;
    photoPicker.nResultType = DO_PICKER_RESULT_UIIMAGE;
    photoPicker.nMaxCount = 1;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.ivPhoto.image = [[AssetHelper sharedAssetHelper] getImageAtIndex:indexPath.row type:ASSET_PHOTO_THUMBNAIL];

	if (_dSelected[[NSString stringWithFormat:@"%ld",(long)indexPath.row]] == nil)
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (_nMaxCount > 1 || _nMaxCount == DO_NO_LIMIT_SELECT)
    {
		...
		if ((_dSelected[[NSString stringWithFormat:@"%ld",(long)indexPath.row]] == nil) && (_nMaxCount > _dSelected.count))
		{
			// select
			_dSelected[[NSString stringWithFormat:@"%ld",(long)indexPath.row]] = @(_dSelected.count);
			[cell setSelectMode:YES];
		}
		else
		{
			// unselect
			[_dSelected removeObjectForKey:[NSString stringWithFormat:@"%ld",(long)indexPath.row]];
			[cell setSelectMode:NO];
		}
    ...
    {
        if (_nResultType == DO_PICKER_RESULT_UIIMAGE)
            [_delegate didSelectPhotosFromDoImagePickerController:self result:@[[ASSETHELPER getImageAtIndex:indexPath.row type:ASSET_PHOTO_SCREEN_SIZE]] :_dSelected];
        else
            [_delegate didSelectPhotosFromDoImagePickerController:self result:@[[ASSETHELPER getAssetAtIndex:indexPath.row]]: _dSelected];
    }
}
